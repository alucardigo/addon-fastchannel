...
public class SankhyaServiceInvoker {
    ...
    private List<String> getServerBaseUrlCandidates() {
        ...
        // 1. Prioridade: URL configurada no banco (AD_FCCONFIG.SANKHYA_SERVER_URL)
        String configUrl = config.getSankhyaServerUrl();
        if (configUrl != null && !configUrl.trim().isEmpty()) {
            candidates.add(configUrl.trim());
        }
        ...
        // 4. Fallbacks localhost (desenvolvimento)
