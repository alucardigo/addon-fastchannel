...
public class CrossComponentIntegrationTest {
    ...
    @Test
    public void headerMapping_tipNeg_fallsBackToConfig_whenPaymentMethodNotMapped() throws Exception {
        OrderDTO order = new OrderDTO();
        ...
        // TipNeg lookup retorna null -> deve usar fallback da config
        FastchannelHeaderMappingService.TipNegLookup tipNeg = paymentMethodId -> null;
        ...
        m.setAccessible(true);
        BigDecimal result = (BigDecimal) m.invoke(service, order);
        assertEquals("Deve usar fallback da config quando PaymentMethodId nao mapeado",
                new BigDecimal("99"), result);
    }
